...
- **決定**:
  ...
  - `MetricsRegistry` で `compress_ratio` / `semantic_retention` を Gauge として保持し、`GET /metrics` から Prometheus Text Format で露出する。
- **影響**:
  - Chainlit ルートに副作用なくサブマウントでき、CI テスト (`pytest`) で監視エンドポイントが検証される。
  - 今後 `semantic_retention` の精度向上（埋め込み導入）時も、Registry 経由で値を差し替え可能。
