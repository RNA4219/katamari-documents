...
### ブランチとレビューのチェックポイント
- ブランチ作成時に「目的・対象ファイル・テスト計画」を Issue または `TASK.*.md` に記載し、AI 支援を活用した場合はプロンプト概要も添えてください。
- AI が提案した変更を採用する場合、コミット本文に「Co-authored-by: Katamari AI <bot@katamari.dev>」を付与し、人間が意図と差分を確認したことを記録します。
- 人間メンテナーはレビュー開始前に `git diff --stat origin/main...HEAD` を共有し、PR のスコープ逸脱があれば Draft へ戻すかサブタスク化します。
...
### テスト・リンターの必須条件
- 上表すべてが対象外の場合でも `ruff check .` と `pytest -q` は変更内容に関わる限り必須です。Node 領域の変更は `pnpm run lint` と `pnpm run test` の双方を実行してください。
- 部分的にスキップする際は `pytest -k` や `pnpm run test -- <pattern>` を利用できますが、PR 説明欄に対象範囲・理由・フォローアップの要否を明記します。
- make タスクを追加した場合は `make help` に説明を加え、`make check` が新しいワークフローで失敗しないことを確認します。
- テストやリンターが失敗した場合は、原因分析と修正方針を PR テンプレートの Notes セクションで共有し、再実行結果も追記してください。
...
2. テンプレートの `Metadata` セクション（番号・作成日・作成者）と `Context`/`Decision`/`Consequences`/`Status`/`DoD` をすべて埋めます。作成日は正確な日付が取得できない場合に限り省略できます。背景リンク・採用理由・DoD チェックリストは必須で、マイルストーン ADR の場合は `docs/katamari_wbs.csv` と整合させます。
...
- PR 作成時は以下を満たしてください。
  ...
  - 「Pairing」: 人間と AI の役割分担（設計・実装・検証）を一行で示し、再現ログの所在をリンクします。

### PR 運用とレビュー判断
- PR は 1 つの課題に集中させ、別課題が発生した場合は TODO コメントで切り出しつつ Issue を起票してからフォローします。
- Ready for review に移行する条件は「テストログ添付」「lint/type チェック済み」「コードオーナー通知済み」の 3 点です。いずれか欠ける場合は Draft を維持してください。
- レビューコメントで合意した修正は 24 時間以内に対応し、遅れる場合は遅延理由と再開予定をコメントします。
- マージ時は `squash` を推奨しますが、TDD の履歴を残す必要がある場合に限り `rebase` を選択できます。その際はコミットログにテスト結果を引用してください。
