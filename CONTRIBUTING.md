...
#### ブランチ運用ガードレール
- main 直下にコミットするのは緊急リリースタグ作業のみとし、例外時は `@katamari-org/maintainers` の事前合意をコメントで残してください。
- ブランチのライフサイクル（作成→ドラフト PR→レビュー→マージ→削除）を Issue でリンクし、どの段階かを常に `Draft` / `Ready` ラベルで共有します。
- 2 営業日以上更新のないブランチはオーナーがステータスを確認し、不要なら削除、継続なら再計画をコメントで宣言します。
...
#### テスト / リンター実行記録
- すべての PR で `pytest -q`、`ruff check .`、`mypy --strict` の最新実行結果を PR テンプレートの `## Test` と `## Checklist` に記載し、日時とコミット SHA を明示します。
- フロントエンドや Node.js を変更した場合は `pnpm run lint`、`pnpm run test` の結果も併記し、対象ディレクトリが無関係ならその理由を記載します。
- CI 失敗を検知したら 1 時間以内に調査状況を PR コメントへ追記し、再実行時はログ差分が分かるよう前回との相違点をまとめてください。
...
#### PR テンプレ適用フロー
1. ドラフト PR を作成し、テンプレート内の全項目を暫定入力した状態で `Draft` のまま共有します。
2. 主要な議題・確認事項は `## Notes` に箇条書きで追加し、担当レビュアと期日を記入します。
3. テスト・Lint を完了したら結果ログを `## Test` に貼り付けて `Ready for review` へ変更し、未完了項目は `## Checklist` に ❌ と期日を残します。
4. 追加コミットで情報が更新された場合はテンプレート内容も追記または修正し、レビュアが履歴を追わなくても状況を把握できるようにします。
...
#### レビュー実施手順
1. PR 作成者は `Ready for review` へ移行する際にレビュー期限、優先度、想定リスクを冒頭コメントへ明記します。
2. CODEOWNERS は 1 営業日以内にレビュー着手可否をコメントし、難易度が高い場合は共同レビュアを指名します。
3. 指摘事項は「必須」「任意」をラベル化し、再現ステップまたは参照コードを添えてください。議論が必要な場合はスレッドを分割し論点を明確化します。
4. 修正後の再レビューでは、レビュアは差分確認時に `Viewed` を付与し、ブロッカー解消を確認したら `Approve` を提出します。未解決の場合は `Request changes` を継続します。
5. マージ担当者は最終確認として CI 成功、必要承認の取得、テンプレートの更新状況をチェックし、完了後にブランチ削除とリリースノート反映を行います。
### ADR を追加・更新する手順
